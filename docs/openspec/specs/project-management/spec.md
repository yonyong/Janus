...
### Requirement: 编辑项目映射
平台 SHALL 允许管理员修改已登记项目的名称与本地目录路径，未提交的字段保持不变；新的目录路径必须是真实存在的目录，改动后立即作为该项目的文件访问与 agent 工作目录基准。

#### Scenario: 修改项目信息
- **WHEN** 管理员提交该项目的新名称或新的本地目录路径
- **THEN** 系统更新映射并返回最新记录，未提交的字段保持原值

#### Scenario: 新路径无效时拒绝
- **WHEN** 管理员把目录路径改成不存在或不是目录的路径
- **THEN** 系统拒绝更新并提示路径无效，原映射保持不变
